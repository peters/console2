...
boost::shared_ptr<BackgroundImage> ImageHandler::GetImage(const ImageData& imageData)
{
	boost::shared_ptr<BackgroundImage>	bkImage(new BackgroundImage(imageData));
	...
	if (!LoadImage(bkImage)) return boost::shared_ptr<BackgroundImage>();
...
boost::shared_ptr<BackgroundImage> ImageHandler::GetDesktopImage(ImageData& imageData)
{
	if (!GetDesktopImageData(imageData)) return boost::shared_ptr<BackgroundImage>();
...
void ImageHandler::UpdateImageBitmap(const CDC& dc, const CRect& clientRect, boost::shared_ptr<BackgroundImage>& bkImage)
{
...
bool ImageHandler::GetDesktopImageData(ImageData& imageData)
{
	...
	if (keyColors.QueryStringValue(L"Background", strBackground.GetBuffer(dwDataSize), &dwDataSize) != ERROR_SUCCESS)
	{
		...
		return boost::shared_ptr<BackgroundImage>();
	}
	strBackground.ReleaseBuffer();
...
bool ImageHandler::LoadImage(boost::shared_ptr<BackgroundImage>& bkImage)
{
	USES_CONVERSION;
...
void ImageHandler::CreateRelativeImage(const CDC& dc, boost::shared_ptr<BackgroundImage>& bkImage)
{
	CriticalSectionLock	lock(bkImage->updateCritSec);
...
void ImageHandler::CreateImage(const CDC& dc, const CRect& clientRect, boost::shared_ptr<BackgroundImage>& bkImage)
{
	CriticalSectionLock	lock(bkImage->updateCritSec);
...
void ImageHandler::PaintTemplateImage(const CDC& dcTemplate, int nOffsetX, int nOffsetY, DWORD dwSrcWidth, DWORD dwSrcHeight, DWORD dwDstWidth, DWORD dwDstHeight, boost::shared_ptr<BackgroundImage>& bkImage)
{
...
void ImageHandler::TileTemplateImage(const CDC& dcTemplate, int nOffsetX, int nOffsetY, boost::shared_ptr<BackgroundImage>& bkImage)
{
...
void ImageHandler::TintImage(const CDC& dc, boost::shared_ptr<BackgroundImage>& bkImage)
{
	CDC				dcTint;
	CBitmap			bmpTint;
